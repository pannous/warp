...
use wasp::test_utils::{read_bytes_fast, cache_stats};
use wasp::eq;
...
fn test_module_cache_reuse() {
    ...
    assert_eq!(bytes1, bytes2, "Identical nodes should produce identical bytecode");
...
fn test_different_modules_not_cached() {
    ...
    assert_ne!(bytes1, bytes2, "Different nodes should produce different bytecode");
...
fn test_cache_statistics() {
    ...
    assert!(misses_after > misses_before, "Should have at least one cache miss");

    // Second access - cache hit
    read_bytes_fast(&bytes).expect("Read failed");
    let (hits_after, _, hit_rate) = cache_stats();
    assert!(hits_after > hits_before, "Should have at least one cache hit");

    println!("Cache statistics: {} hits, {} misses, {:.1}% hit rate",
             hits_after, misses_after, hit_rate * 100.0);
}
