...
fn assert ! Nil() {
assert ! (Empty.isNil());
eq ! (Empty.name.data, nil_name);
assert ! (nil_name == "nil"s); // WASM
if (Empty.name.data == nil_name);
eq ! (Empty.name, nil_name);
# [cfg(not(feature = "LINUX"))]{ // WHY???
assert ! (Empty.name.data == nil_name);
}
assert ! (Empty.length == 0);
assert ! (Empty.children == 0);
assert ! (Empty.parent == 0);
assert ! (Empty.next == 0);
}
...
#[test]
fn testRoots() {
    assert!(Empty.value.longy == 0);
    // is!((char *) "'hello'", "hello");
    skip!(
is!("hello", "hello", 0)); // todo reference==string really?
    is!("True", True);
    is!("False", False);
    is!("true", True);
    is!("false", False);
...
fn testAllEmit() {
 ...
 // todo!
    ...
    assert!(Empty.value.longy == 0); // should never be modified
